...
<div class="max-w-5xl space-y-6 mb-12">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h3 class="font-extrabold text-2xl text-gray-900">Chỉnh sửa: {{ $post->title }}</h3>
                <p class="text-xs text-gray-500">Cập nhật thông tin và nội dung bài viết</p>
            </div>
        </div>
    </div>

    <!-- Form -->
    <form action="{{ route('admin.posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="md:col-span-2">
                ...
                <input type="text" name="title" required value="{{ old('title', $post->title) }}" class="w-full text-base font-bold rounded-xl border border-gray-300 p-3.5 focus:ring-brand-500 focus:border-brand-500 shadow-xs text-gray-900 placeholder:font-normal placeholder:text-gray-400" />
            </div>
        ...
        <div>
            ...
            <input type="text" name="slug" required value="{{ old('slug', $post->slug) }}" class="w-full font-mono text-xs text-gray-600 rounded-xl border border-gray-300 p-3 focus:ring-brand-500 focus:border-brand-500 shadow-xs bg-gray-50/50" />
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-5 bg-gray-50/50 rounded-2xl border border-gray-100 items-center">
            <div>
                <label class="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">Ảnh đại diện (Upload thay thế file mới)</label>
                <div class="flex items-center space-x-4 mb-2">
                    @if($post->thumbnail)
                        <img src="{{ $post->thumbnail }}" class="w-16 h-12 object-cover rounded-xl border border-gray-300 shadow-inner" alt="">
                    @endif
            ...
            <div>
                ...
                <input type="text" name="thumbnail" value="{{ old('thumbnail', $post->thumbnail) }}" placeholder="https://..." class="w-full font-mono text-xs rounded-xl border border-gray-300 p-3 bg-white shadow-xs focus:ring-brand-500 focus:border-brand-500" />
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="excerpt" rows="3" class="w-full text-sm rounded-xl border border-gray-300 p-3.5 focus:ring-brand-500 focus:border-brand-500 shadow-xs leading-relaxed text-gray-700">{{ old('excerpt', $post->excerpt) }}</textarea>
        </div>
        ...
        <div>
            ...
            <div class="rounded-2xl shadow-sm border border-gray-200">
                <textarea id="post_editor" name="content">{{ old('content', $post->content) }}</textarea>
            </div>
        </div>
        ...
        <div class="pt-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="flex items-center space-x-6">
                <div class="flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox" name="is_published" value="1" id="is_pub" {{ old('is_published', $post->is_published) ? 'checked' : '' }} class="w-5 h-5 text-brand-600 rounded border-gray-300 focus:ring-brand-500 cursor-pointer">
                    <label for="is_pub" class="text-sm font-bold text-gray-800 cursor-pointer">Xuất bản ngay (Hiển thị công khai)</label>
                </div>
                <div class="flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox" name="is_featured" value="1" id="is_feat" {{ old('is_featured', $post->is_featured) ? 'checked' : '' }} class="w-5 h-5 text-amber-500 rounded border-gray-300 focus:ring-amber-400 cursor-pointer">
